...
import { PillIcon, WarningIcon, ScanIcon, ShareIcon, ClipboardIcon } from './Icons';
...
const PrescriptionResult: React.FC<Props> = ({ data, onReset, autoPlaySuccess }) => {
  ...
  const generateShareText = () => {
    let text = `📋 *AarogyaVani Prescription Summary*\n\n`;
    
    // Medicines
    text += `💊 *Medicines:*\n`;
    data.structured_data.medicines.forEach((med, idx) => {
      text += `${idx + 1}. ${med.name} - ${med.dosage} - ${med.timing}\n`;
      if (med.notes) text += `   Note: ${med.notes}\n`;
    });
    text += `\n`;
    ...
    if (interactions.length > 0) {
      ...
      interactions.forEach(i => {
        text += `- ${i.severity} Risk: ${i.medicines.join('+')} (${i.description})\n`;
      });
      text += `\n`;
    }

    // Script
    text += `🗣️ *Instructions (${languageName}):*\n`;
    text += `${data.voice_script_native}\n\n`;

    // Disclaimer
    text += `(Generated by AI. Not medical advice. Verify with a doctor.)`;
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: 'Prescription Summary - AarogyaVani',
          text: text,
        });
      } catch (err) {
  ...
  return (
    <div className="w-full max-w-3xl mx-auto space-y-8 animate-fade-in-up">
      ...
      <div className="flex justify-end px-2">
        <button 
          onClick={handleShare}
          className="flex items-center gap-2 bg-white px-4 py-2 rounded-full shadow-sm border border-slate-200 text-teal-700 font-medium hover:bg-teal-50 transition-colors"
        >
            {copyFeedback ? (
                <>
